...
from qiskit import QuantumRegister, QuantumCircuit, BasicAer, Aer
...
def qgan_top_noisy(**kwargs):
    ...
    qgan_noise_model = my_utils.get_noise_model(p_noise)
    # from qiskit.test.mock import FakeVigo
    # from qiskit.providers.aer.noise import NoiseModel
    # fake_backend = FakeVigo()
    # noise_model = NoiseModel.from_backend(fake_backend)

    backend = AerSimulator(method="statevector", noise_model=qgan_noise_model)

    # Set quantum instance to run the quantum generator
    quantum_instance = QuantumInstance(backend=backend)

    g_circuit_noisy = transpile(g_circuit, backend)
    #g_circuit_noisy = copy.deepcopy(g_circuit)
    g_circuit.decompose().draw(output='mpl')
    g_circuit_noisy.decompose().draw(output='mpl')

    # Set quantum generator
    qgan.set_generator(generator_circuit=g_circuit_noisy, generator_init_params=init_params)
    # The parameters have an order issue that following is a temp. workaround
    qgan._generator._free_parameters = sorted(g_circuit_noisy.parameters, key=lambda p: p.name)
